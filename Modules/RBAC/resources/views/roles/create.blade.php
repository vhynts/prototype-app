...
<div class="max-w-5xl">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-xs border border-slate-100 dark:border-slate-800 overflow-hidden">
        <div class="px-6 py-5 border-b border-slate-100 dark:border-slate-800">
            <h2 class="text-lg font-bold text-slate-900 dark:text-white">Create New Role</h2>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Define a new role and assign its specific permissions.</p>
        </div>

        <form method="POST" action="{{ route('admin.roles.store') }}" class="p-6 space-y-8" @submit="submitting = true" x-data="{
            submitting: false,
            ...
            <div class="flex items-center justify-end gap-3 pt-6 border-t border-slate-100 dark:border-slate-800 mt-8">
                ...
                <button type="submit" :disabled="submitting"
                    ...
                    <span x-text="submitting ? 'Saving...' : 'Save Role'">Save Role</span>
                </button>
            </div>
        </form>
    </div>
