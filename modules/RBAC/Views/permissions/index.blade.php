...
<div class="max-w-5xl">
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-slate-500 mt-1">View the complete list of available system access rights.</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <div class="px-5 py-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <form method="GET" action="{{ route('admin.permissions.index') }}" class="relative max-w-sm w-full">
                ...
                <input type="text" name="search" value="{{ request('search') }}" class="block w-full pl-9 pr-3 py-2 border border-slate-200 rounded-xl bg-slate-50 text-sm font-medium placeholder-slate-400 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:bg-white transition-colors" placeholder="Search permissions...">
                @if(request('search'))
                    <a href="{{ route('admin.permissions.index') }}" class="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 hover:text-slate-600">
                ...
                <tbody class="text-sm">
                    @forelse ($permissions as $permission)
                        ...
                        <tr class="border-b border-slate-50 last:border-0 hover:bg-slate-50/80 transition-colors">
                            ...
                            <td class="px-5 py-4">
                                <span class="inline-flex items-center rounded-md bg-slate-50 border border-slate-200 px-2 py-0.5 text-[10px] font-bold text-slate-600 uppercase tracking-wide">
                                    {{ $group }}
                                </span>
                            </td>
                            <td class="px-5 py-4 text-right">
                                <span class="inline-flex items-center justify-center rounded-lg bg-indigo-50 border border-indigo-100 px-2.5 py-0.5 text-[11px] font-bold text-indigo-700 uppercase tracking-wide">
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="px-5 py-8 text-center">
                                ...
                                <h3 class="text-sm font-bold text-slate-900 mb-1">No Permissions Found</h3>
        ...
        <div class="px-5 py-4 border-t border-slate-100 flex items-center justify-between">
            <span class="text-xs font-medium text-slate-500">
                Showing <span class="font-bold text-slate-900">1</span> to <span class="font-bold text-slate-900">{{ $permissions->count() }}</span> of <span class="font-bold text-slate-900">{{ $permissions->total() }}</span> results
            </span>
        </div>
        @endif
    </div>
